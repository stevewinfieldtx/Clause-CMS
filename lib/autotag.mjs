...
// Pure inline-formatting tags (no editable href of their own). When one of these
// sits inside a heading/paragraph it is ABSORBED into that element's rich field
// instead of fragmenting it — so the whole line stays editable as one field.
// <a>/<button> are intentionally NOT here: they own an editable href, so they
// must remain independent fields.
const INLINE_FORMAT = new Set(['em', 'strong', 'b', 'i', 'span', 'br', 'small', 'sup', 'sub', 'mark', 'code', 'u', 'abbr', 'time']);
...
export function autotag(rawHtml, baseUrl, startN = 0) {
  ...
  $('*').each((_, el) => {
    ...
    // Already absorbed into a tagged rich field above (e.g. a <span> inside a
    // tagged <h1>) — don't re-tag it as its own separate field.
    if ($(el).parents('[data-cms]').length) return;
    // A descendant that holds its own text makes this a container — skip it — BUT
    // pure inline-formatting tags (strong/em/span/code…) don't count: they get
    // absorbed into this element's rich field. <a>/<button> DO count, so a real
    // sub-element with its own href still becomes its own field.
    let hasBlockLeafChild = false;
    $(el).find('*').each((_, d) => { if (!INLINE_FORMAT.has(d.tagName) && isTextLeaf($, d)) hasBlockLeafChild = true; });
    if (hasBlockLeafChild) return;
  ...
  // EMBEDS — iframes explicitly marked data-cms-embed (from the block picker), OR
  // recognized by src as a YouTube/Vimeo player — a real ingested page's existing
  // video embeds never carry the marker attribute, so src-sniffing is what makes
  // them editable at all.
  const EMBED_HOST_RE = /(?:^|\/\/)(?:www\.)?(?:youtube(?:-nocookie)?\.com\/embed\/|player\.vimeo\.com\/)/i;
  $('iframe[data-cms-embed], iframe[src]').each((_, el) => {
    const src = $(el).attr('src'); if (!src) return;
    if (!$(el).is('[data-cms-embed]') && !EMBED_HOST_RE.test(src)) return;
    if ($(el).attr('data-cms-embed')) return;  // already tagged (e.g. duplicate selector match)
    const id = `cms-${++n}`;
