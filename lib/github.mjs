...
export async function pushFilesToBranch({ repo, branch, base = 'main', files, message }) {
  ...
  try {
    ...
    // force: the CMS always pushes its FULL current rendered state, never a
    // diff — so if another sync raced this one and moved the branch first,
    // this later, equally-authoritative snapshot is meant to win, not be
    // rejected as a non-fast-forward. (queueRepoSync/repoPublish also
    // serialize per site so this really is a last-resort safety net.)
    const upd = await gh('PATCH', refPath(repo, branch).replace('/ref/', '/refs/'), { sha: commit.json.sha, force: true });
