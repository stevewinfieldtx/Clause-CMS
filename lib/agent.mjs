...
- {"op":"set","id":"cms-N","value":"..."} — change one content slot's text/image URL/embed URL. PREFER this for pure copy/image changes: it is the most precise tool.
...
- {"op":"set_css","css":"..."} — replace the page's custom stylesheet (a dedicated <style> owned by you). You get its current contents; emit the FULL replacement, not a delta.
...
- NO <script>, event handlers (onclick=…), javascript: URLs, <form>/<input>, or iframes other than YouTube/Vimeo embeds. They will be stripped.
    ...
    properties: {
      ...
      ops: {
        ...
        items: {
          type: 'object',
          properties: {
            op: { type: 'string', enum: ['set', 'replace_section', 'insert_section', 'remove_section', 'move_section', 'set_css'] },
            id: { type: 'string' },
            value: { type: 'string' },
