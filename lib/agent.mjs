...
import { fieldHandles } from './fields.mjs';

export { plannerMode };

function catalogue(schema, content, handles = { byId: {}, urls: {} }) {
  // Every field carries its human-readable "ref" ("Heading 2", "Text 6", "Video 1",
  // "URL 3") — the SAME name the client sees in the editor — so a request like
  // "change Video 1" or "change URL 3" maps to exactly one id.
  const items = Object.entries(schema).map(([id, d]) => ({
    id, ref: handles.byId[id] || null,
    label: d.label, group: d.group, type: d.type,
    rich: d.rich ? d.allow : false,
    current: String(content[id] ?? '').slice(0, 120),
  }));
  // Link/button destinations are first-class "URL N" fields the planner can set.
  for (const [cid, info] of Object.entries(handles.urls || {})) {
    const d = schema[cid]; if (!d) continue;
    items.push({
      id: `link:${cid}`, ref: info.ref,
      label: `Destination URL for ${handles.byId[cid] || d.label}`,
      group: d.group, type: 'url', rich: false,
      current: String(content[`link:${cid}`] ?? info.href).slice(0, 120),
    });
  }
  return items;
}
...
- Every field has a "ref": a short human name the client uses, like "Heading 2", "Text 6", "Link 4", "Button 1", "Image 11", "Video 1", or "URL 3". When the client names a ref, edit exactly that field's id. "First/second video" maps to "Video 1"/"Video 2"; if they don't say which of several, use the lowest-numbered one.
- You MUST emit a "set" op for every change you describe. NEVER write a summary that claims a change (e.g. "Changed the video") while leaving "changes" empty — if you cannot confidently pick a field, emit no summary of success and say in "summary" exactly what you need to proceed.
- Fields with "type" of "image", "embed" or "url" accept ONLY a URL as their value — never words ("video 1" is an embed URL; a "URL N" field is a link/button destination like https://…, /path, #anchor, mailto: or tel:). For wording, spelling or capitalization changes, target text fields only. If the words the client wants changed are part of an image (e.g. lettering baked into a logo), return no changes and explain in "summary" that the image itself would need to be replaced.
...
async function openrouterPlan(command, content, schema, creds, history = [], handles = { byId: {}, urls: {} }) {
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        ...history,
        { role: 'user', content: `Editable fields (catalogue):\n${JSON.stringify(catalogue(schema, content, handles))}\n\nClient request: "${command}"` },
      ],
...
function localPlan(command, content, schema, handles = { byId: {}, urls: {} }) {
  const value = extractValue(command);
  // Direct handle hit: "change Video 1 to …", "set URL 3 to …". The ref names the field.
  const cl = command.toLowerCase();
  const refMap = {};
  for (const [id, ref] of Object.entries(handles.byId || {})) refMap[String(ref).toLowerCase()] = id;
  for (const [id, info] of Object.entries(handles.urls || {})) refMap[String(info.ref).toLowerCase()] = 'link:' + id;
  if (value != null) {
    for (const [ref, id] of Object.entries(refMap)) {
      const i = cl.indexOf(ref);
      // require a non-digit boundary so "Video 1" doesn't match "video 10"
      if (i >= 0 && !/\d/.test(cl[i + ref.length] || '')) {
        return { summary: `Set ${ref} to "${value}".`, changeset: [{ op: 'set', id, value }] };
      }
    }
  }
...
export async function plan(command, content, schema, history = [], templateHtml = '') {
  const creds = aiCreds();
  const handles = fieldHandles(templateHtml, schema);
  try {
    if (creds.provider === 'openrouter') return await openrouterPlan(command, content, schema, creds, history, handles);
    return localPlan(command, content, schema, handles);
  } catch (e) {
    return { summary: `Planner error: ${e.message}. Falling back to local matching.`, changeset: localPlan(command, content, schema, handles).changeset };
  }
}
