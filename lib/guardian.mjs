...
import { hasInjection, sanitizeRich, plainify, isSafeImageUrl, isSafeEmbedUrl, normalizeEmbedUrl } from './sanitize.mjs';
...
export function validate(changeset, site) {
  ...
  for (const change of changeset) {
    ...
    if (def.type === 'image') {
      ...
      // Clients paste whatever YouTube/Vimeo link they have; normalize to the
      // canonical /embed/ URL an <iframe> can load, then hold it to the allowlist.
      const norm = normalizeEmbedUrl(value);
      if (!isSafeEmbedUrl(norm)) { errors.push(`${def.label}: "${String(value).slice(0, 60)}" is not a valid YouTube or Vimeo link.`); continue; }
      if (norm !== String(value).trim()) warnings.push(`${def.label}: converted to its embed link (${norm}).`);
      clean = norm;
    } else if (def.rich) {
  ...
  if (Object.keys(applied).length) {
    try {
      ...
      $('[data-cms-embed]').each((_, el) => present.add(el.attribs['data-cms-embed']));
