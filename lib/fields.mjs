...
/**
 * Give every editable field a short, stable, human-readable handle
 * ("Heading 2", "Text 6", "Link 4", "Button 1", "Image 11", "Video 1", "URL 3"),
 * numbered per kind in document order. This is the ONE source of truth shared by
 * the AI planner's catalogue and the editor UI, so the client and the AI name a
 * field the exact same way. Returns:
 *   { byId:  { <cms-id>: "<handle>" },                 // content fields
 *     urls:  { <cms-id>: { ref: "URL N", href } } }    // link/button destinations
 */
export function fieldHandles(templateHtml, schema = {}) {
  const $ = load(String(templateHtml || ''), { decodeEntities: false });
  const n = {};
  const next = (kind) => `${kind} ${(n[kind] = (n[kind] || 0) + 1)}`;
  const byId = {};
  const urls = {};
  $('[data-cms], [data-cms-img], [data-cms-embed]').each((_, el) => {
    const $el = $(el);
    const t = (el.tagName || '').toLowerCase();
    let id, kind;
    if ($el.is('[data-cms-embed]')) { kind = 'Video'; id = $el.attr('data-cms-embed') || $el.attr('data-cms'); }
    else if ($el.is('[data-cms-img]')) { kind = t === 'video' ? 'Video' : 'Image'; id = $el.attr('data-cms-img') || $el.attr('data-cms'); }
    else {
      id = $el.attr('data-cms');
      const cls = $el.attr('class') || '';
      if (/^h[1-6]$/.test(t)) kind = 'Heading';
      else if (t === 'button' || $el.closest('button').length) kind = 'Button';
      else if (t === 'a' || $el.closest('a').length) kind = /\bbtn\b/.test(cls) ? 'Button' : 'Link';
      else kind = 'Text';
    }
    if (!id || byId[id]) return;
    byId[id] = next(kind);
    // A link/button destination gets its own "URL N" handle (a virtual link:<id> field).
    const a = $el.is('a') ? $el : $el.closest('a');
    if (a.length && schema[id]) urls[id] = { ref: next('URL'), href: a.attr('href') || '' };
  });
  return { byId, urls };
}
