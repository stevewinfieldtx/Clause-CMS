...
/** Embed src: only YouTube and Vimeo embed URLs are allowed. */
export function isSafeEmbedUrl(value) {
  const v = String(value).trim();
  return /^https:\/\/(www\.)?(youtube\.com\/embed\/|youtu\.be\/|player\.vimeo\.com\/video\/)[\w\-?=&%]+/.test(v);
}

/**
 * Turn any YouTube/Vimeo link a client is likely to paste — the address-bar
 * watch URL, a youtu.be share link, a Shorts/live link, or the plain vimeo.com
 * page — into the canonical /embed/ form an <iframe> can actually load. Already
 * canonical or unrecognized input is returned trimmed but unchanged, so the
 * caller still validates the result with isSafeEmbedUrl.
 */
export function normalizeEmbedUrl(value) {
  const v = String(value).trim();
  // Already an embed player URL — leave as-is.
  if (/^https:\/\/(www\.)?(youtube\.com\/embed\/|youtube-nocookie\.com\/embed\/|player\.vimeo\.com\/video\/)/i.test(v)) return v;
  // YouTube: watch?v=, /shorts/, /live/, or youtu.be short link. IDs are 11 chars.
  let m = v.match(/^https?:\/\/(?:www\.|m\.)?youtube\.com\/(?:watch\?(?:[^#]*&)?v=|shorts\/|live\/|v\/)([\w-]{11})/i)
       || v.match(/^https?:\/\/youtu\.be\/([\w-]{11})/i);
  if (m) return `https://www.youtube.com/embed/${m[1]}`;
  // Vimeo: vimeo.com/123456789
  m = v.match(/^https?:\/\/(?:www\.)?vimeo\.com\/(\d+)/i);
