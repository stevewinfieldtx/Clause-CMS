  ...
  for (const [id, def] of Object.entries(schema)) {
    ...
    if (def.type === 'image') {
      if (isSafeImageUrl(val)) {
        // A <video>'s image slot is its POSTER. Writing src on a <video>
        // overrides its <source> children — the browser then tries to "play"
        // a JPEG and the hero video freezes to a still.
        $(`[data-cms-img="${id}"]`).each((_, el) => {
          $(el).attr(el.tagName?.toLowerCase() === 'video' ? 'poster' : 'src', val);
        });
        // Repair templates already poisoned by the old behaviour.
        $(`video[data-cms-img="${id}"][src]`).removeAttr('src');
      }
      continue;
    }
    if (def.type === 'embed') {
    ...
    if (def.rich) el.html(sanitizeRich(val, def.allow));
      ...
      // The tagged element wraps other elements (e.g. <label>Your name<input></label>):
      // replace only its text nodes so the wrapped controls survive the edit.
      el.contents().each((_, n) => { if (n.type === 'text') $(n).remove(); });
      el.prepend(plainify(val).replace(/&/g, '&amp;').replace(/</g, '&lt;'));
